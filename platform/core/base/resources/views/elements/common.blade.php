        ...
        <script type="text/javascript">
            $(function() {
                @if (Session::has('success_msg'))
                    Botble.showSuccess('{{ BaseHelper::clean(addslashes(session('success_msg'))) }}');
                @endif
                @if (Session::has('error_msg'))
                    Botble.showError('{{ BaseHelper::clean(addslashes(session('error_msg'))) }}');
                @endif
                @if (isset($error_msg))
                    Botble.showError('{{ BaseHelper::clean(addslashes($error_msg)) }}');
                @endif
                @if (isset($errors))
                    @foreach ($errors->all() as $error)
                        Botble.showError('{{ BaseHelper::clean(addslashes($error)) }}');
                    @endforeach
                @endif
            })
        </script>
