...
class HookServiceProvider extends ServiceProvider
{
            ...
            add_filter(PAYMENT_FILTER_PAYMENT_DATA, function (array $data, Request $request) {
                ...
                $rooms = [
                    [
                        ...
                        'price' => $this->convertOrderAmount($booking->amount + $booking->tax_amount - $booking->coupon_amount),
                        'price_per_order' => $this->convertOrderAmount($booking->amount),
                        'qty' => 1,
                    ],
                ];
                ...
                return [
                    'amount' => $this->convertOrderAmount((float)$booking->amount),
                    'shipping_amount' => 0,
                    'shipping_method' => null,
                    'tax_amount' => $this->convertOrderAmount($booking->tax_amount),
                    'discount_amount' => $this->convertOrderAmount($booking->coupon_amount),
    ...
    public function countPendingBookings(int|string|null $number, string $menuId): string|null
    {
        if ($menuId == 'cms-plugins-booking') {
            $attributes = [
                'class' => 'badge bg-primary text-primary-fg menu-item-count pending-bookings',
                'style' => 'display: none;',
            ];
