@if (is_plugin_active('payment'))
    <link href="{{ asset('vendor/core/plugins/payment/css/payment.css') }}?v=1.0.3" rel="stylesheet">
            ...
            <div class="col-lg-8">
                <div class="booking-form-body room-details">
                    <form class="booking-form-main payment-checkout-form"
                        ...
                        <input name="start_date" type="hidden" value="{{ $startDate->format('d-m-Y') }}">
                        <input name="end_date" type="hidden" value="{{ $endDate->format('d-m-Y') }}">
                        <input name="adults" type="hidden" value="{{ $adults }}">
                        <input name="currency" type="hidden"
                            value="{{ strtoupper(get_application_currency()->title) }}">
                        <input name="currency_id" type="hidden" value="{{ get_application_currency_id() }}">
                        @if (is_plugin_active('paypal'))
                            <input name="callback_url" type="hidden" value="{{ route('payments.paypal.status') }}">
                        @endif

                        <input name="number_of_guests" type="hidden" value="{{ $adults }}">
                        ...
                        <div class="comboContainer">
                            ...
                            <!-- 1st Room -->
                            <div class="comboTable activeCombo">
                                <div class="flex30 roomDetails">
                                    <h6>Room Only</h6>
                                    <div class="imgBox">
                                        <img src="{{ RvMedia::getImageUrl($room->image, '380x280', false, RvMedia::getDefaultImage()) }}"
                                            alt="">
                                    </div>
                                    {{-- <ul>
                                        <li>⏍ 174sq.ft</li>
                                ...
                                <div class="flex30">
                                    <div class="comboDetailsOff">
                                        <h5>Services</h5>
                                        <ul>
                                          <li><span class="clrRed">𐄂</span> Breakfast</li>
                                          <li><span class="clrRed">𐄂</span> Lunch/Dinner</li>
                                            @php
                                                $with = ['metadata'];
                                ...
                                <div class="flex30 checkCombo">
                                    <h5>Per night for 4 Rooms</h5>
                                    <p><span>Amount</span> <span><del>{{ format_price($amount+500) }}</del> {{ format_price($amount) }}</span></p>
                                    <p><span>Tax</span> <span>{{ format_price($taxAmount) }}</span></p>
                                    <p><span>Total</span> <span>{{ format_price($total) }}</span></p>

                                    {{-- <div class="couponCode">
                                        <input type="text" placeholder="WELCOME10%"> <button>Apply</button>
                                    </div> --}}

                                    <!-- <button class="selectCombo"> SELECT COMBO</button> -->
                                    <div class="combo-btn">
                                        <input class="service-item check-value first" type="checkbox" name="Rooms" value="singleRoom" checked  id="roomonly">
                                        <label for="roomonly">Room only</label>
                                    </div>
                                </div>
                            </div>
                            <!-- 2nd Room -->
                             ...
                             {{-- @if (count($chunks) > 0) --}}
                                @foreach ($services as $service)
                                    <div class="comboTable ">
                                        <div class="flex30 roomDetails">
                                            <h6>Room With {{ $service->name }}</h6>
                                            <div class="imgBox">
                                                <img src="{{ RvMedia::getImageUrl($service->image, '380x280', false, RvMedia::getDefaultImage()) }}"
                                                    alt="">
                                            </div>
                                            {{-- <ul>
                                                <li>⏍ 174sq.ft</li>
                                            ...
                                            <div class="comboDetailsOff">
                                                ...
                                                <ul>
                                                    ...
                                                    @elseif($service->id == 13)
                                                        <li><span class="clrGreen">✓</span> Breakfast</li>
                                                        <li><span class="clrRed">𐄂</span> Lunch/Dinner</li>
                                                    @elseif($service->id == 14)
                                                         <li><span class="clrGreen">✓</span> Breakfast</li>
                                                         <li><span class="clrGreen">✓</span> Lunch/Dinner</li>
                                                    @endif
                                                    @php
                                                        $with = ['metadata'];
                                        ...
                                        <div class="flex30 checkCombo">
                                            <h5>Per night for 1 Rooms</h5>
                                            <p><span>Charge for this Combo</span> <span>{{ format_price($service->price) }}</span></p>
                                            {{-- <p><span>Amount</span> <span><del>{{ format_price($amount+500+$service->price) }}</del> {{ format_price($amount) }}</span></p>
                                            <p><span>Tax</span> <span>{{ format_price($taxAmount) }}</span></p>
                                            <p><span>Total</span> <span>{{ format_price($total+$service->price) }}</span></p> --}}
                                            ...
                                            <!-- <button class="selectCombo"> SELECT COMBO</button> -->
                                            <div class="combo-btn">
                                                <input class="service-item second check-value" id="service_{{ $service->id }}" name="services[]" type="checkbox"  
                                                        value="{{ $service->id }}"
                                                        @if (in_array($service->id, (array) old('services', $selectedServices)))  @endif >
                                                <label for="service_{{ $service->id }}">Select Combo</label>
                                            </div>
                                        </div>
                                    </div>
                                @endforeach
