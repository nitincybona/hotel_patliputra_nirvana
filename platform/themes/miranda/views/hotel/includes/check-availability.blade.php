<div class="room-booking-form">
    ...
    <form action="{{ $availableForBooking ? route('public.booking') : route('public.rooms') }}" method="{{ $availableForBooking ? 'POST' : 'GET' }}">
        ...
        <div class="input-group input-group-two left-icon mb-20">
            ...
            <input type="text" placeholder="{{ request()->query('start_date', now()->format('d-m-Y')) }}" value="{{ request()->query('start_date', now()->format('d-m-Y')) }}" name="start_date" id="arrival-date" class="date-picker" data-date-format="dd-mm-yyyy"
                   data-locale="{{ App::getLocale() }}">
        </div>
        <div class="input-group input-group-two left-icon mb-20">
            <label for="departure-date">{{ __('Check-Out') }}</label>
            <div class="icon"><i class="fal fa-calendar-alt"></i></div>
            <input type="text" placeholder="{{ request()->query('end_date', now()->addDay()->format('d-m-Y')) }}" value="{{ request()->query('end_date', now()->addDay()->format('d-m-Y')) }}" name="end_date" id="departure-date" class="date-picker" data-date-format="dd-mm-yyyy"
                   data-locale="{{ App::getLocale() }}">
        </div>
        <div class="input-group input-group-two left-icon mb-20">
